...
# docker run -v $(pwd)/Spitchd:/var/lib/Spitchd -v $(pwd)/wallet:/home/Spitch --rm -ti Spitch:0.2.2
...
ARG Spitch_BRANCH=master
ENV Spitch_BRANCH=${Spitch_BRANCH}
...
RUN apt-get update && \
    ...
    git clone https://github.com/Spitch/Spitch.git /src/Spitch && \
    cd /src/Spitch && \
    git checkout $Spitch_BRANCH && \
    mkdir build && \
    cd build && \
