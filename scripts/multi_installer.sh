...
# Spitch Multi-installer
# a one line clone-and-compile for Spitch:
#
#     ` $ curl -sL "https://raw.githubusercontent.com/Spitch/Spitch/master/scripts/multi_installer.sh" | bash
#
...
_set_wd() {
    if [ -d "$PWD/.git" ] && [ -f "$PWD/Makefile" ]; then
        ...
        if [ -d "$PWD"/Spitch ]; then
            read -r -p "${1:-Spitch directory already exists. Overwrite? [y/N]} " response
            case "$response" in
                [yY][eE][sS|[yY])
                    _colorize red "Overwriting old Spitch directory" && echo
                    rm -rf "$PWD"/Spitch
                    ;;
                *)
                    _fail "Spitch directory already exists. Aborting..."
                    ;;
            esac
        fi
        mkdir Spitch
        git clone -b master -q https://github.com/Spitch/Spitch Spitch   >>build.log 2>&1 || _fail "Unable to clone git repository. Please see build.log for more information"
        cd Spitch
    fi
}

_build_Spitch() {
    _note "Building Spitch from source (this might take a while)..."
    if [ -d build ]; then
...
_configure_linux() {
    if [ "$(awk -F= '/^NAME/{print $2}' /etc/os-release)" = "\"Ubuntu\"" ]; then
        ...
        _fail "Your OS version isn't supported by this installer. Please consider adding support for your OS to the project ('https://github.com/Spitch')"
    fi
}
...
_configure_os() {
    ...
    case "${_unameOut}" in
        ...
        Darwin*)
            ...
            ;;
        *)
            _fail "This installer only runs on OSX 10.10+ and Ubuntu 16.04+. Please consider adding support for your OS to the project ('https://github.com/Spitch')"
            ;;
    esac
    _note "Operating system configuration completed. You're halfway there!"
}

_note "Spitch Multi_Installer v1.0 (pepperoni)"
...
_note "Look in 'Spitch/build/src/' for the executible binaries. See 'https://github.com/Spitch/Spitch' for more project support. Cowabunga!"
